...
# validations
  ...
  if !cpg_exists?(new_resource.storage_system, new_resource.name)
    op_status = create_cpg(new_resource.storage_system, new_resource.cpg_name, new_resource.domain, new_resource.growth_increment, 
...
# validations
  ...
  op_status = delete_cpg(new_resource.storage_system, new_resource.cpg_name)
  new_resource.updated_by_last_action(true) if op_status
end
