...
# validations
  ...
  if !flash_cache_exists?(new_resource.storage_system)
    op_status = create_flash_cache(new_resource.storage_system, new_resource.size_in_gib, new_resource.mode)
...
# validations
  ...
  op_status = delete_flash_cache(new_resource.storage_system)
  new_resource.updated_by_last_action(true) if op_status
end